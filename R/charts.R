...
create_base_chart <- function(type,
                              hc_thm = get_astho_hc_theme(), # nolint: object_usage_linter
                              export_options = list()) {
  ...
  if (type %in% c("column", "bar", "pie", "bubble", "packedbubble", "sankey")) {
    hc_thm$colors <- apply_pattern_colors(hc_thm$colors) # nolint: object_usage_linter
  }
  highcharter::highchart() |>
    ...
    viz_export(export_options) # nolint: object_usage_linter
}
